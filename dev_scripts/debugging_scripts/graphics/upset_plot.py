...
up_filt = UpsetPlot(
    sets,
    mode='exclusive',
    filter=lambda df: df[df['_size'] >= 1].sort_values('_size', ascending=False),
)
...
up_drop = UpsetPlot(sets, filter=drop_set_d)
print('\n=== after dropping Gene Set D ===')
print(up_drop.intersections)
up_drop.plot()

# ── 5. Deferred render + artist customisation ───────────────────────────
up_custom = UpsetPlot(sets, mode='exclusive', filter=lambda df: df[df['_size'] >= 1])

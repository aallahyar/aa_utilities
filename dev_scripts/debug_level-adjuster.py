...
from my_utilities.computation import adjust_levels
...
def plot_before_after(meta, original, adjusted):
    ...
    sns.stripplot(data=m_cond, x='condition', y='expr', hue='state', dodge=True, color='black', alpha=0.5, ax=axes[0])
    ...
    sns.stripplot(data=m_batch, x='batch', y='expr', hue='state', dodge=True, color='black', alpha=0.5, ax=axes[1])
